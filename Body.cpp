...
void Body::createMesh(ElementType* type)
{
    ...
    for (auto line = mLines.begin(); line < mLines.end(); ++line)
    {
        ...
        if (divisions > 1)
        {
            ...
            Point* point1 = mPoints[connect[0]];
            Point* point2 = mPoints[connect[1]];
    ...
    int numberofsegments = mLines.size();

    std::vector<int> v2;
    for (auto segment = mLines.begin(); segment < mLines.end(); ++segment)
    {
    ...
    mMesh->nodesNum = out->numberofpoints;
