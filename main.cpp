...
int main()
{
    ...
    // Preparation before meshing
    for (int i = 0; i < currentModel->getBody(bodyName)->getLinesNum(); ++i)
        currentModel->getBody(bodyName)->setLineDivisions(i, 10);
    ...
    currentModel->setPartMaterial(partName, materName);
    // Select element type for part
